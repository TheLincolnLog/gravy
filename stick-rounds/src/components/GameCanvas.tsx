...
export const GameCanvas: React.FC<GameCanvasProps> = ({ players, onWin, onDamage, onUpdatePlayer, gamePhase, level, socket }) => {
  ...
  useEffect(() => { onDamageRef.current = onDamage; }, [onDamage]);
  ...
  useEffect(() => {
    ...
    engine.onWeaponChange = (id, weapon) => onUpdatePlayerRef.current(id, { currentWeapon: weapon });
    engine.onForceFieldChange = (id, active) => onUpdatePlayerRef.current(id, { hasForceField: active });
    engine.onGodModeChange = (id, pellets, mode, end) => onUpdatePlayerRef.current(id, { godPellets: pellets, activeGodMode: mode, godModeEnd: end });
    ...
    if (socket) {
      socket.on('network_update', (data: any) => {
        if (engineRef.current && data.playerId) {
          const remotePlayer = playersRef.current.find(p => p.id === data.playerId);
          if (remotePlayer && remotePlayer.controllerId !== socket.id) {
            engineRef.current.syncNetworkPlayer(data.playerId, data.pos, data.vel, data.health);
          }
        }
      });
    }
    ...
    const animate = () => {
      if (engineRef.current) {
        ...
        // Emit local player states
        if (socket && engineRef.current && Date.now() % 3 === 0) { // Throttled emit
           playersRef.current.forEach(p => {
             if (p.controllerId === socket.id) {
                const posVel = engineRef.current?.getPlayerPosVel(p.id);
                if (posVel) {
                   socket.emit('network_update', { 
                     lobbyId: (socket as any).lobbyId || 'GLOBAL', // We should track lobbyId
                     data: {
                       playerId: p.id,
                       pos: posVel.pos,
                       vel: posVel.vel,
      ...
      <div className="absolute inset-0 pointer-events-none p-8 md:p-12 flex flex-col justify-between z-20">
        <div className="flex justify-between items-start">
             {players.slice(0, 2).map((p, i) => (
                  <div key={p.id} className={`flex flex-col gap-2 w-64 md:w-80 ${i === 1 ? 'items-end ml-auto text-right' : ''}`}>
                    <div className="flex justify-between w-full text-sm font-black uppercase tracking-[0.2em]" style={{ color: p.color }}>
                        {i === 0 ? (
                            <><span>{p.name}</span><span className="text-white/60">{Math.round(p.stats.health)}%</span></>
                        ) : (
                            <><span>{Math.round(p.stats.health)}%</span><span className="text-white/60">{p.name}</span></>
                        )}
                    </div>
                    <div className="h-3 w-full bg-white/5 border border-white/10 relative overflow-hidden rounded-full">
                        <motion.div 
                            initial={{ width: '100%' }}
                            animate={{ width: `${(p.stats.health / p.stats.maxHealth) * 100}%` }}
                            className="h-full" 
                            style={{ backgroundColor: p.color, boxShadow: `0 0 15px ${p.color}aa` }}
                        />
                    </div>
                    {/* God Pellets Row */}
                    <div className={`flex gap-3 mt-1 ${i === 1 ? 'flex-row-reverse' : 'flex-row'}`}>
                        {(['red', 'orange', 'yellow', 'green', 'blue', 'purple', 'pink'] as const).map(color => {
                            const count = p.godPellets?.[color] || 0;
                            const hex = { red: '#ef4444', orange: '#f97316', yellow: '#facc15', green: '#22c55e', blue: '#3b82f6', purple: '#d946ef', pink: '#f472b6' }[color];
                            return (
        ...
        <div className="flex justify-between items-end">
             {players.slice(2, 4).map((p, i) => (
                  <div key={p.id} className={`flex flex-col gap-2 w-64 md:w-80 ${i === 1 ? 'items-end ml-auto text-right' : ''}`}>
                    {/* God Pellets Row */}
                    <div className={`flex gap-3 mb-1 ${i === 1 ? 'flex-row-reverse' : 'flex-row'}`}>
                        {(['red', 'orange', 'yellow', 'green', 'blue', 'purple', 'pink'] as const).map(color => {
                            const count = p.godPellets?.[color] || 0;
                            const hex = { red: '#ef4444', orange: '#f97316', yellow: '#facc15', green: '#22c55e', blue: '#3b82f6', purple: '#d946ef', pink: '#f472b6' }[color];
                            return (
