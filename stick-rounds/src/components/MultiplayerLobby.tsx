...
export const MultiplayerLobby: React.FC<Props> = ({ onGameStart, playerName }) => {
  ...
  useEffect(() => {
    ...
    newSocket.on('lobby_created', (data: LobbyData) => {
      ...
      setLobby(data);
      setActivePlayerId(newSocket.id);
      setError(null);
    });
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-7xl px-8 py-12">
      ...
      <div className="mt-16 flex flex-col items-center gap-6">
        {lobby.owner === socket?.id ? (
          <button 
            ...
            `}
          >
            {playersInLobby.every((p: Player) => p.lockedArchetype) ? 'Initialize Protocol' : 'Awaiting Squad Lock'}
          </button>
        ) : (
          <div className="text-center">
            <span className="text-[10px] font-black uppercase opacity-40 tracking-[0.5em] animate-pulse">Waiting for Host Activation</span>
          </div>
        )}
      </div>
    </div>
