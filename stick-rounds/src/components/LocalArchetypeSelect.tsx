...
import { StickmanSilhouette } from './StickmanSilhouette';
...
export const LocalArchetypeSelect: React.FC<Props> = ({ players, onComplete }) => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-7xl px-8 py-12 bg-black/40 backdrop-blur-md border-x border-white/5 min-h-[80vh]">
      <div className="flex justify-between items-center w-full mb-12">
        <div className="flex flex-col">
          <span className="text-[10px] font-black tracking-[0.5em] text-white/30 uppercase">Protocol // Archetype Selection</span>
          <h2 className="text-4xl font-black italic uppercase tracking-tighter flex items-center gap-4">
            <Shield className="text-cyan-400" /> PLAYER {currentPlayerIdx + 1} SELECTING
          </h2>
        </div>

        <div className="flex gap-4">
          {players.map((p, idx) => (
            <div key={p.id} className={`flex flex-col items-end transition-all ${idx === currentPlayerIdx ? 'opacity-100 scale-110' : 'opacity-40'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full">
        {ARCHETYPES.map(arch => {
          ...
          return (
            <motion.div 
              key={arch.id}
              onMouseEnter={() => setHoveredIdx(arch.id)}
      ...
      <div className="mt-16 flex flex-col items-center gap-6">
        {allSelected ? (
          <button 
            onClick={finalize}
            className="px-24 py-8 bg-white text-black -skew-x-12 font-black text-4xl italic uppercase transition-all hover:scale-110 shadow-2xl flex items-center gap-4"
          >
            INITIALIZE PROTOCOL <ChevronRight size={32} />
          </button>
        ) : (
