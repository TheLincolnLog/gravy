  ...
  const handleWin = useCallback((winnerId: number, deathOrder: number[]) => {
    ...
    setPlayers((prev) => {
      const nextPlayers = prev.map((p) => {
        ...
        const newScore = isWinner ? p.score + 1 : p.score;
        if (newScore >= 10) finalWinner = { ...p, score: newScore };
        return {
          ...p,
          score: newScore,
    ...
    <div className="min-h-screen bg-[#0C0C0C] text-white flex flex-col items-center justify-center font-sans relative border-8 border-[#1A1A1A] overflow-hidden">
          ...
          >
            <div className="flex flex-col">
              ...
              <span className="text-[9px] font-bold text-white/30 tracking-[0.5em] uppercase mt-1">
                {matchName} // {players.length}P AUTHENTICATED
              </span>
            </div>
      ...
      <AnimatePresence mode="wait">
        {phase === "menu" && (
          ...
          >
            <h2 className="text-[12rem] font-black tracking-tighter uppercase italic leading-[0.8] mb-12 text-center select-none">
              CUBE
              <br />
              CLASH
            </h2>
        ...
        {(phase as any) === "matchOver" && (
          ...
          >
            ...
            <h2 className="text-[10rem] font-black italic uppercase tracking-tighter leading-none mb-4" style={{ color: players.find(p => p.score >= 10)?.color }}>
              {players.find(p => p.score >= 10)?.name}
            </h2>
