...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedMode) {
      ...
      setControlMode(isMobile ? 'mobile' : 'pc');
    }
      ...
      <AnimatePresence mode="wait">
        {phase === "menu" && (
          ...
          >
            ...
            >
              <h2 className="text-6xl sm:text-8xl md:text-[10rem] lg:text-[12rem] font-black tracking-tighter uppercase italic leading-[0.8] text-center select-none relative z-10">
                CUBE
                <br />
                CLASH
              ...
              <div className="clash-faders">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="clash-fader text-6xl sm:text-8xl md:text-[10rem] lg:text-[12rem] font-black tracking-tighter uppercase italic leading-[0.8] text-white/10 text-center select-none">
                    CUBE<br />CLASH
                  </div>
                ))}
              </div>
        ...
        {phase === "playerSelect" && (
          <motion.div
            ...
            animate={{ opacity: 1, x: 0 }}
            className="flex flex-col items-center z-10 w-full max-w-4xl px-4 sm:px-8"
          >
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-black italic uppercase tracking-tighter mb-8 sm:mb-16 flex items-center gap-4 sm:gap-6">
              <Users className="w-10 h-10 sm:w-[60px] sm:h-[60px]" /> HOW MANY CUBES?
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12 w-full">
              {[2, 3, 4].map((count) => (
                <button
                  ...
                  className="group relative aspect-[3/2] md:aspect-square bg-[#111] border-4 border-white/5 hover:border-white transition-all flex flex-col items-center justify-center overflow-hidden"
                >
                  <div className="absolute inset-0 bg-white opacity-0 group-hover:opacity-10 transition-opacity" />
                  <span className="text-6xl sm:text-8xl md:text-[12rem] font-black italic opacity-10 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0">
                    {count}
                  </span>
                  <span className="text-[10px] sm:text-sm font-black tracking-[0.3em] sm:tracking-[0.5em] uppercase opacity-40 group-hover:opacity-100">
                    Competitive Cubes
                  </span>
                </button>
              ))}
        ...
        {(phase as any) === "matchOver" && (
          ...
          >
            ...
            <h2 className="text-4xl sm:text-6xl md:text-[10rem] font-black italic uppercase tracking-tighter leading-none mb-4" style={{ color: players.sort((a,b) => b.score - a.score)[0]?.color }}>
              {players.sort((a,b) => b.score - a.score)[0]?.name}
            </h2>
            <div className="text-2xl sm:text-4xl font-black italic uppercase tracking-widest mb-12">CHAMPION</div>
            <div className="flex flex-col sm:flex-row gap-6 sm:gap-12 mb-16">
                {players.sort((a,b) => b.score - a.score).map((p, i) => (
                    <div key={p.id} className="flex flex-col items-center">
                        ...
                        <span className="text-2xl sm:text-3xl font-black italic uppercase" style={{ color: p.color }}>{p.score} PTS</span>
                    </div>
                ))}
            </div>
            <button
               ...
               className="px-8 py-4 sm:px-16 sm:py-6 bg-white text-black font-black text-lg sm:text-2xl uppercase italic -skew-x-12 hover:scale-105 transition-all w-full sm:w-auto"
            >
              RETURN TO MENU
            </button>
          </motion.div>
